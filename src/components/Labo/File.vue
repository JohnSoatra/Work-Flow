...
export default {
    ...
    methods: {
        onDoubleClick(evt) {
            this.$emit("onDoubleClick", evt);
        },
        onMouseDown(evt) {
            const location = { name: this.name, index: this.index }
            this.$emit("onMouseDown", evt, location);
        }
    }
}
</script>
